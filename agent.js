...
const client = new OpenAI({
  ...
  timeout: 5 * 60 * 1000,
  defaultHeaders: {
    "HTTP-Referer": "http://localhost:3000", // Wajib ada untuk beberapa router (bisa diisi domain Anda jika sudah prod)
    "X-Title": "Meridian SOL", // Nama aplikasi Anda bebas
    "User-Agent":
      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", // Menyamarkan sebagai browser agar tidak diblok WAF
...
function isThinkingModeToolChoiceError(error) {
  ...
  return (
    /thinking mode does not support/i.test(message) &&
...
export async function agentLoop(
  ...
  const mustUseRealTool = shouldRequireRealToolUse(
    goal,
    agentType,
    interactive,
  );
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      let toolChoice =
        step === 0 && (ACTION_INTENTS.test(goal) || mustUseRealTool)
          ? "required"
          : "auto";
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          if (!omitToolChoice && isThinkingModeToolChoiceError(error)) {
            ...
            log(
              "agent",
              "Provider thinking mode does not support tool_choice — retrying without it",
            );
            attempt -= 1;
            continue;
          }
